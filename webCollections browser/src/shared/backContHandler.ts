...
export default class BackgroundHandler {
  ...
  private handleMessage(message: Ops.Operation): void {
    switch (message.operation) {
      ...
      case Ops.OperationType.collections:
        if (this.collectionsHandler)
          this.collectionsHandler(
            (message as Ops.CollectionsOperation).collections
          );
        break;

      case Ops.OperationType.mapCheckResults:
        {
          const check = message as Ops.MapCheckResultsOperation;
          if (this.mapCheckResultsHandler)
            this.mapCheckResultsHandler(
              check.mapId,
              check.available,
              check.mapCollections
            );
        }
        break;
      ...
      case Ops.OperationType.collectionMaps:
        {
          const colMaps = message as Ops.CollectionMapsOperation;
          if (this.collectionsMapsHandler)
            this.collectionsMapsHandler(
              colMaps.collection,
              colMaps.collectionSize,
              colMaps.maps
            );
        }
        break;
      ...
      case Ops.OperationType.collectionMapAdd:
        {
          ...
          if (this.collectionMapAddHandler)
            this.collectionMapAddHandler(addM.collection, addM.mapId);
        }
        break;
      ...
      case Ops.OperationType.collectionMapRemove:
        {
          ...
          if (this.collectionMapRemoveHandler)
            this.collectionMapRemoveHandler(remM.collection, remM.mapId);
        }
        break;
  ...
  public collectionsMapsHandler:
    | ((collection: string, size: number, maps: [Beatmap]) => void)
    | undefined;

  public collectionMapAddHandler:
    | ((collection: string, mapId: string) => void)
    | undefined;

  public collectionMapRemoveHandler:
    | ((collection: string, mapId: string) => void)
    | undefined;
